...
Options:
	- [parts to run]    	can assume values: 'all', 'star', 'dcc', 'fuchs', 'dcc_indep'
...
Examples:
	./circtools_master.sh ~/testing/ fuchs /mnt/schratt/mouse_RNAseR/raw mouse paired default "mouse_GRCm38_RNAseR"
	./circtools_master.sh ~/testing dcc /mnt/schratt/mouse_control_hippocampus/ mouse unpaired 12 ""
...
if [ $2 = "dcc" ] || [ $2 = "dcc_indep" ] || [ $2 = "all" ]; then
	cd $work_dir &&
	# create directories
	if [ $2 = "dcc_indep" ]; then
		mkdir -pv $work_dir/dcc &&
	else
		mkdir -pv circtools &&
	...
	if [ $2 = "dcc_indep" ]; then
		cd [sS][tT][aA][rR]*/
		parallel --plus ln -s `pwd`/{}.bam ../dcc/{}.bam :::: ../.tmp/names.tmp
		parallel --plus ln -s `pwd`/{}.bam.bai ../dcc/{}.bam.bai :::: ../.tmp/names.tmp
		parallel --plus ln -s `pwd`/{}.chimeric
